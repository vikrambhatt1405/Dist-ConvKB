...
parser.add_argument("--use_pretrained", default=False, type=bool, help="Using the pretrained embeddings")
parser.add_argument("--num_epochs", default=201, type=int, help="Number of training epochs")
parser.add_argument("--saveStep", default=50, type=int, help="")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def train_step(x_batch, y_batch):
            ...
            _, step, loss_summary = sess.run([train_op, global_step, cnn.loss_summary], feed_dict)
            return step,loss_summary

        num_batches_per_epoch = int((data_size - 1) / args.batch_size) + 1
        for epoch in range(args.num_epochs):
            for batch_num in range(num_batches_per_epoch):
                x_batch, y_batch = train_batch()
                step,loss_summary=train_step(x_batch, y_batch)
                current_step = step #tf.train.global_step(sess, global_step)
                if(current_step % 100 == 0):
                    fileWriter.add_summary(loss_summary,step)
            if epoch > 0:
                if epoch % args.saveStep == 0:
                    path = cnn.saver.save(sess, checkpoint_prefix, global_step=epoch)
                    print("Saved model checkpoint to {}\n".format(path))
        print("Total number of global steps till end:",current_step)
        print("Total number of training steps till end:",step)
